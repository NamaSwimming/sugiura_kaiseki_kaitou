...
\begin{tleftbar}
    ...
    この条件のもとで，
    \[
    M = \max_{1 \leq k \leq m} \abs{f(a_k)}
    \]
    と定義する．また任意の分割 $\Delta$ に対して，
    \[
    s(f; \Delta, \xi) = \sum_{k=1}^{n} f(\xi_k) v(I_k)
    \]
    であるが，$\xi_k$ が $a_1, a_2, \ldots, a_m$のいずれかである場合のみ$f(\xi_k) \ne 0$であるから，
    リーマン和に寄与する項は高々$m$個である．ゆえに．
    \[
    \abs{s(f; \Delta, \xi)} = \abs{\sum_{k=1}^{n} f(\xi_k) v(I_k)} \leq m M \sum_{k=1}^{n}v(I_k).
    \]
    さて，$d(\Delta)$を分割$\Delta$の直径とすると，$v(I_k) \leq d(\Delta)^n$だから，
    
    \[
    \abs{s(f; \Delta, \xi)} \leq m M d(\Delta)^n
    \]
    ここで，$d(\Delta) \to 0$ とすると，$\abs{s(f; \Delta, \xi)} \to 0$となる．
    したがって，$f$は区間$I$上でリーマン積分可能であり，
