...
\subsection*{p2：1-(i\romannumeral9)}
...
$x$についての$n$次多項式 $H_n (x)$の隣り合う$2$根を$a < b$としたとき，
\[
    H_n'(a) H_n'(b)<0.
\]
...
\begin{leftbar}
    まず定義
    \[
        \frac{d^n}{dx^n}\bigl(e^{-x^2}\bigr)
        \;=\;
        (-1)^n\,e^{-x^2}\,H_n(x)
    \]
    を確認する．左辺は $e^{-x^2}$ に $n$ 回の微分を施したものなので，右辺の $H_n(x)$ は明らかに $x$ の多項式成分となる．さらに最高次の微分を考えると，$x^n$ の項が最上位で残ることから，$H_n(x)$ は次数 $n$ の多項式であることが分かる．
