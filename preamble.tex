...
% 共通のボックススタイルを定義
\newtcolorbox{leftbarbox}[1]{%
  enhanced,
  breakable,
  frame hidden,
  boxrule=0pt,
  borderline west={1.1mm}{0pt}{gray!70}, % 左のバー
  colback=white,
  left=5mm, right=2mm, top=2mm, bottom=2mm,
  pad at break=1mm,
  before skip=\topsep, after skip=\topsep,
  %--- タイトル設定 ---
  attach boxed title to top left={xshift=0.5\zw, yshift*=-\fboxsep, yshift*=-0.5pt},
  ...
  title={#1} % 環境の引数からタイトルを直接設定
}

%--- ユーザーが使うための環境定義 ---

% \begin{tproof}[見出し] ... \end{tproof}
% amsthmのQED機能と連携
\NewDocumentEnvironment{tproof}{ O{証明} }{%
  \pushQED{\qed}% amsthmのQEDスタックを開始
  \begin{leftbarbox}{#1}%
    }{%
    \popQED% QED記号を配置
  \end{leftbarbox}%
...
% \begin{tanswer}[見出し] ... \end{tanswer}
% QEDなしのバージョン
\NewDocumentEnvironment{tanswer}{ O{解答} }{%
  \begin{leftbarbox}{#1}%
    }{%
  \end{leftbarbox}%
}
